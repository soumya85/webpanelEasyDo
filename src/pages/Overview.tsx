...
const generateKPIData = (dateRange: DateRange) => {
  ...
  const multiplier = Math.min(daysDiff / 30, 2);
...
const generateWorkingHourTrendsData = (dateRange: DateRange) => {
  ...
  for (let i = 1; i <= 30; i++) {
    ...
    data.push({
      day: i,
      Present: Math.floor(6 + random() * 3),
      Leave: Math.floor(random() * 2),
      Absent: Math.floor(random() * 1),
      Holiday: i % 7 === 0 ? Math.floor(1 + random()) : 0,
      OT: Math.floor(random() * 3),
      RedFlags: Math.floor(random() * 2),
    });
  }
  return data;
};
