...
export const TodayScheduleCard: React.FC<TodayScheduleCardProps> = ({
  ...
  onResize,
    ...
    // Filter by tag
    if (selectedTag === "my") {
        filteredTasks = filteredTasks.filter((task) => task.tags.includes("G.Task") || task.tags.includes("Group Task"));
    } else if (selectedTag === "delegated") {
        filteredTasks = filteredTasks.filter((task) => task.tags.includes("Delegated"));
    }
        ...
        <DashboardCard id={id} index={index} size={size} onResize={onResize}>
            ...
            <div className="px-6 pt-5 pb-3 border-b flex items-center justify-between bg-gradient-to-r from-blue-50 to-indigo-50">
                ...
                <div className="flex flex-col items-end">
                    <span className="text-xs font-semibold text-blue-700">{getDateDisplay(selectedDate)}</span>
            ...
            <div className="flex gap-3 px-6 py-2 border-b bg-white">
                {days.map((d, i) => (
                    ...
                    >
                        ...
                        <span className="font-bold">{parseInt(d.date.split("-")[2], 10)}</span>
            ...
            <div className="px-6 py-4 bg-[#F8FAFF]">
                ...
                <div className="max-h-72 overflow-y-auto pr-2">
                    {filteredTasks.length === 0 ? (
                        <div className="text-center text-gray-400 text-sm py-8">No tasks for this day and filter.</div>
                    ) : (
                        filteredTasks.map((task) => (
                            <div key={task.id} className="bg-blue-100 rounded-xl p-4 mb-4 flex flex-col gap-1 shadow-sm">
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="bg-green-500 text-white text-xs font-bold px-2 py-1 rounded-lg">{task.daysLeft}d</span>
                                </div>
